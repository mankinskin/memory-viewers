...
pub(super) fn canvas_size() -> (f64, f64) {
    if let Some((width, height)) = web_sys::window()
        ...
        .map(|element| (element.client_width() as f64, element.client_height() as f64))
...
pub(super) fn subscribe_sse(
    ...
    let upsert_listener = gloo_events::EventListener::new(&event_source, "edge.upsert", move |_| {
        trigger_upsert.with_mut(|value| *value += 1);
    });
    let delete_listener = gloo_events::EventListener::new(&event_source, "edge.delete", move |_| {
